...
// 1. FIREBASE CONFIG NA FIX YA PRIVATE KEY
...
const serviceAccount = {
  ...
  "private_key": rawKey.replace(/\\n/g, '\n'), // Hii line ndio inafuta hiyo error
...
async function startBot() {
    ...
    // Tunatumia collection moja kuzuia "doc is not a function" error
    const sessionRef = db.collection("WT6_SESSIONS");
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
            console.log("WRONG TURN 6: CONNECTED ✔️");
    ...
    sock.ev.on('call', async (node) => {
        if (node[0].status === 'offer') {
            ...
            await sock.sendMessage(node[0].from, { text: "📵 *SECURITY:* WRONG TURN 6 calls are auto-blocked." });
        }
    });
}
